...
const codeToType= (code) =>
  Object.entries(weatherTypes).reduce(
    (currentType,[type,codes])=>
      codes.includes(Number(code)) ? type : currentType
  )


const WeatherIcon = ({moment, currentWeatherCode }) => {
  const [currentType, setCurrentType]= useState('isClear')
  const weatherIcon = useMemo(() => 
    codeToType(currentWeatherCode)
  , [currentWeatherCode])
  useEffect(()=>{
    setCurrentType(weatherIcon)
  },[weatherIcon])
  return (
    <IconContainer>
      {weatherIcons[moment][currentType]}
    </IconContainer>
  );
};
